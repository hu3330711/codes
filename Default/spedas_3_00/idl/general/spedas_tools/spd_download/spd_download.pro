...
;$URL: svn+ssh://thmsvn@ambrosia.ssl.berkeley.edu/repos/spdsoft/tags/spedas_3_00/general/spedas_tools/spd_download/spd_download.pro $
;
;-
...
if ~keyword_set(file_mode) then file_mode='777'o
...
if ~keyword_set(no_download) and ~keyword_set(no_wildcards) then begin
  ...
  url_original = url
  spd_download_expand, url, last_version=last_version, ssl_verify_peer=ssl_verify_peer, ssl_verify_host=ssl_verify_host,port=port, _extra=_extra
