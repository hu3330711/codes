...
;$URL: svn+ssh://thmsvn@ambrosia.ssl.berkeley.edu/repos/spdsoft/tags/spedas_3_00/general/spedas_tools/spd_download/spd_download_expand.pro $
;
;-

pro spd_download_expand, url, last_version=last_version, ssl_verify_peer=ssl_verify_peer, ssl_verify_host=ssl_verify_host,port=port, _extra=_extra
...
if n_remote gt 0 then begin
  ...
  for i=0, n_elements(unique_bases)-1 do begin
    ...
    current = spd_download_file(url=unique_bases[i], /string_array, ssl_verify_peer=ssl_verify_peer, ssl_verify_host=ssl_verify_host ,port=port, _extra=_extra)
