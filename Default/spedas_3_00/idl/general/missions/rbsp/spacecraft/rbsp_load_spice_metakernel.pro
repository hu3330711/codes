...
;   $URL: svn+ssh://thmsvn@ambrosia.ssl.berkeley.edu/repos/spdsoft/tags/spedas_3_00/general/missions/rbsp/spacecraft/rbsp_load_spice_metakernel.pro $
;
;-


pro rbsp_load_spice_metakernel, metakernel, unload=unload,update=update,all=all,trange=trange
...
while not eof(unit) do begin
  ...
  if inblock then begin
    ...
    if kerneltoload ne '' then begin
      ...
      files = spd_download(remote_file=kerneltoload, remote_path=!rbsp_spice.REMOTE_DATA_DIR, no_update=no_update,local_path = !rbsp_spice.local_data_dir, ssl_verify_peer=0, ssl_verify_host=0, /last_version )
