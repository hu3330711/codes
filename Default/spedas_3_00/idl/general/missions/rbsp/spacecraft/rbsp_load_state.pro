...
compile_opt idl2, hidden
  nsm = 500 ;X.C. Shen change it from 1000 to 500
  for i = 0, 2 do $
    for j = 0, 2 do $
...
pro rbsp_load_state, probe = probe, datatype = datatype, dt = dt, $
  ...
  no_eclipse = no_eclipse, $
  use_eph_predict = use_eph_predict, $
  no_spice_load = no_spice_load,trange=trange
...
if ~keyword_set(trange) then tspan = timerange() else tspan=time_double(trange)
