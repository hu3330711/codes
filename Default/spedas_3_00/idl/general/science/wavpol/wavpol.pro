...
; $URL: svn+ssh://thmsvn@ambrosia.ssl.berkeley.edu/repos/spdsoft/tags/spedas_3_00/general/science/wavpol/wavpol.pro $
;-
pro wavpol,ct,Bx,By,Bz,timeline,freqline,powspec_out,degpol,waveangle,elliptict,helict,pspec3,$
samp_per=samp_per_input, nopfft=nopfft_input,steplength = steplength_input, bin_freq = bin_freq_input
    ...
    accuracy = 0.1 ; the accuracy of the sampling frequency should be about 1 percent.
    ...
    powspec_out=Make_Array(nosteps,nopfft/2,/double)
    elliptict=Make_Array(nosteps,nopfft/2,/double)  ;added 2018-10-05 sxc
    helict=Make_Array(nosteps,nopfft/2,/double)
    ...
    for batch = 0l, n_batches-1 do begin
      ...
      if(ngood Gt nopfft) then begin
          ...
          for j=0L,nbp_fft_batches[batch]-1L do begin        
              ...
              ;-------since original wavepower is varying with window size, this powerspec output is replced by FFT calculation of x,y,z components respectively. Modified by X.C. Shen on 20181205
              bign=nopfft
              dbign=double(nopfft)
              window=smooth
              xs2 = abs(fft(tempx, 1))^2
              ys2 = abs(fft(tempy, 1))^2
              zs2 = abs(fft(tempz, 1))^2
              ;x component
              pwrx = dblarr(bign/2)
              pwrx[0] = xs2[0]/dbign^2
              pwrx[1:bign/2-2] = (xs2[1:bign/2-1] + xs2[bign - dindgen(bign/2-2)])/dbign^2
              pwrx[bign/2-1] = xs2[bign/2-1]/dbign^2
              w=(dbign/total(window^2))
              pwrx = w*pwrx/binwidth
              ;y component
              pwry = dblarr(bign/2)
              pwry[0] = ys2[0]/dbign^2
              pwry[1:bign/2-2] = (ys2[1:bign/2-2] + ys2[bign - dindgen(bign/2-2)])/dbign^2
              pwry[bign/2-1] = ys2[bign/2-1]/dbign^2
              w=(dbign/total(window^2))
              pwry = w*pwry/binwidth
              ;z component
              pwrz = dblarr(bign/2)
              pwrz[0] = zs2[0]/dbign^2
              pwrz[1:bign/2-2] = (zs2[1:bign/2-2] + zs2[bign - dindgen(bign/2-2)])/dbign^2
              pwrz[bign/2-1] = zs2[bign/2-1]/dbign^2
              w=(dbign/total(window^2))
              pwrz = w*pwrz/binwidth
              ;sum
              pwr=pwrx+pwry+pwrz
              powspec_out[KK,*]=pwr
              ;------end here--------------------------------------
      ...
      Endif Else Begin
          ...
          powspec_out[KK,1:nopfft/2-2]=!values.d_nan
          powspec_out[KK,0]=!values.d_nan
          powspec_out[KK,nopfft/2-1]=!values.d_nan
    ...
    if zerocount ne 0 then begin
      ...
      powspec_out[wherezero, *] = !values.d_nan
