;Author: ryurow $
;Date: 2017/01/09 22:48:51 $
;Header: /home/cdaweb/dev/control/RCS/orbit_plt.pro,v 1.170 2017/01/09 22:48:51 ryurow Exp ryurow $
;Locker: ryurow $
;Revision: 1.170 $
...
; Fudge to allow more than 10 plots (will we ever need this?) Ron Yurow (12/14/15)
kks = (kks - 1) MOD 10 + 1
...
; Reorder the tags in a structure based on array of 'weights' for each tag that
; is passed as a parameter. The third parameter indicates whether the tags will
; be reordered in ascending (non-zero) or descending order (0).  The default is
; ascending.
; Added by Ron Yurow (Jan 9, 2017)
PRO rebuild_vstruct, s, weight, dir

    ; Default direction for ordering is ascending if none is specified.
    IF N_ELEMENTS (dir) eq 0 THEN dir = 1

    ; Sort the weight array to get an initial order to sort the tags.
    order = SORT (weight)

    ; If we are doing a descending sort, then reverse this.
    IF dir eq 0 THEN order = REVERSE (order)

    ; Number of tags in the structure.
    ntags = N_TAGS (s) 

    ; And the tag names.
    tags  = TAG_NAMES (s)

    ; new will be re-ordered version of the original structure, s.
    ; Start by creating it so that it contains the first tag.
    new = CREATE_STRUCT (tags [order [0]], s.(order [0]))

    ; Loop through all the remaining tags and each one in turn to new
    ; structure.
    FOR i = 1, ntags - 1 DO BEGIN

        new = CREATE_STRUCT (new, tags (order [i]), s.(order [i]))

    ENDFOR

    ; Finished. Copy back new to s.
    s = new
...
; Assign colors to spacecraft orbits being ploted based on the number of spacecraft and 
; if any of the spacecraft are part of MMS.
color_scale = get_ssc_colors ((TAG_NAMES (astrc))[1:*])

; Re-arrange the astrc structure so that all spacecraft orbits to be plotted are ordered
; in descending order based on the color they will be plotted in. 
rebuild_vstruct, astrc, [1000, color_scale], 0

; Re-arrange the color scale array to match the order of missions in the astrc structure.
color_scale = color_scale [REVERSE (SORT (color_scale))]
...
;     Force this to be black, no matter what color the first plot is.
;     Ron Yurow (Jan 9, 2017)
;     xyouts,0.35,ytpos+.015,'symbols mark s/c at end of time range!C',charsize=1.2,/normal,color=fix(s_prts[1])
     xyouts,0.35,ytpos+.015,'symbols mark s/c at end of time range!C',charsize=1.2,/normal,color=5
...
; New adjusting title
     ...
     ; here!!!
     xyouts,0.5,yp,title1,charsize=chtsize0,align=.5,/normal,color=bkgrd
; end title
