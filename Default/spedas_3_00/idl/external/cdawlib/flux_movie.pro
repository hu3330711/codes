;author: $ 
;Date: 2015/07/23 17:36:57 $
;Header: /home/cdaweb/dev/control/RCS/flux_movie.pro,v 1.26 2015/07/23 17:36:57 johnson Exp ryurow $
;Locker: ryurow $
;Revision: 1.26 $
;+------------------------------------------------------------------------
; NAME: FLUX_MOVIE
...
; Initialize temp_gifs to !NULL to make it easy to add images to the array as they
; are created. 
; Ron Yurow (August 5, 2016)
temp_gifs = !NULL
...
;original settings for EUV	 /noborder, /nocircles, /nodipole, /nocolorbar, gif=temp_gif)	
  ...
  if (stat ge 0) then begin ;good image found and put in temp_gif gif file.
    ...
    ; This will not work if the first image is not useable.  Initializing the temp_gifs
    ; variable to !NULL solves the problem of needing a special case for adding the first
    ; Ron Yurow (August 5, 2016)
    ; if (numgif eq 0) then temp_gifs = temp_gif else temp_gifs = [temp_gifs,temp_gif]
    temp_gifs = [temp_gifs,temp_gif]
