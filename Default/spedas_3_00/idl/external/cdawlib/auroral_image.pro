...
;
;  Id: auroral_image.pro,v 1.86 2013/01/11 20:02:50 kovalick Exp ryurow $
;
...
if (n_elements(centerLonLat) le 0 or compute_lonlat) then begin
   ...
   if is1dim then centerLat = long(Lat1(Zsize[1]/2 - long(sqrt(Zsize[1])/2.))) $
      ...
      ; Adjusted the limits that determine at which point the while loop exits if a valid 
      ; lattitude is not found.  Because Lat1 is accessed after dum1/dum2 are incremented,
      ; the proper limits for exiting the loop is not size (dum1/dum2) but 
      ; size (dum1/dum2) - 1.  Note that this does not fix the problem of what happens if 
      ; a valid lattitude is not found.
      ; Ron Yurow (August 23, 2016)  
      while (isbad(dum1/2,dum2/2) ne 0) and (dum1/2 lt zsize[1]-1) and (dum2/2 lt zsize[2]-1) do begin
	 dum1=dum1+1
	 dum2=dum2+1
         centerLat = long(Lat1[dum1/2,dum2/2])
      endwhile
