;Author: johnson $
;Date: 2015/10/06 15:08:09 $
;Header: /home/cdaweb/dev/control/RCS/evaluate_varstruct.pro,v 1.71 2015/10/06 15:08:09 johnson Exp ryurow $
;Locker: ryurow $
;Revision: 1.71 $
...
;;TJK		   if (dn[0] ne ' ') then pstruct.npanels = n_elements(dn)
		   ...
		   if (b[0] ne -1) then begin
			...
			if (dsize[0] le 1) then begin ;scalar
			  ...
			  if (count lt 1) then pstruct.npanels = 1 ; RCJ 10/05/2015 no good values to plot so plot empty plot w/ msg inside.
			  ...
			  if (count lt 1) then begin
			    ; RCJ 10/01/2015  Will leave this line commented out in case we need
			    ;  this again, but we want to put a warning in the center of plot
			    ;  panel if there are no good values to plot, not simply remove the panel
...
; Added display type to handle audiofication.  
