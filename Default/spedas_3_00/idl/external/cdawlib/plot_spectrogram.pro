;Author: ryurow $
;Date: 2017/06/20 20:51:24 $
;Header: /home/cdaweb/dev/control/RCS/plot_spectrogram.pro,v 1.124 2017/06/20 20:51:24 ryurow Exp ryurow $
;Locker: ryurow $
;Revision: 1.124 $
;+------------------------------------------------------------------------
...
; Remove Epochs that are fillval.  This avoids some corner cases with the following
; section of code that removes overlapping times.
; Ron Yurow (June 20, 2017)
; Get good epochs (not fill value)
good_epoch = WHERE (times ne Xfillval, good_cnt)
; and how many epochs need to be removed.
nremove = N_ELEMENTS (times) - good_cnt
; and dimensions of the data.
dim = SIZE (thedata, /DIMENSIONS)
; check the data dimensions.  we can only handle 2D and 3D
CASE N_ELEMENTS (dim) OF
     2: BEGIN  ; 2D
        ; Check if all times are good.
        IF  nremove eq 0 THEN BREAK
        ; Adjust time and data
        times =   times   [good_epoch]
        thedata = thedata [*, good_epoch]
        END

     3: BEGIN  ; 3D
        ; Check if all times are good.
        IF  nremove eq 0 THEN BREAK
        ; Decide which dimension reprsents time, then adjust the data
        time_dim = WHERE (dim eq N_ELEMENTS (times), select_count)
        CASE time_dim [select_count - 1] OF
             0: thedata = thedata [good_epoch, *, *]
             1: thedata = thedata [*, good_epoch, *]
             2: thedata = thedata [*, *, good_epoch]
        ENDCASE
        ; Adjust time
        times = times [good_epoch]
        END

     ELSE : BEGIN ; too many dimensions
        IF KEYWORD_SET (DEBUG) THEN PRINT, "Could not remove NULL values from EPOCH."
        nremvoe = 0
        END
ENDCASE

IF KEYWORD_SET (DEBUG) && nremove gt 0 THEN PRINT, "Removed : ", nremove, " epochs." 
...
if n_elements(times) ne 1 then begin
   ...
   ; need to also add logic here to check for fill value when checking to see if all
   ; times are already good prior to beginning the loop processing.
   ; Ron Yurow  (June 19, 2017)
   ; q=where(nt le 0) ; where no good data
   q=where(nt le 0 and ntimes ne Xfillval)
   ...
   while done eq 0 do begin
      ...
      if n_elements(dim) eq 3 then begin ; 3D
       qq=where(dim eq n_elements(times), nqq)
       if (nqq gt 0) then begin
          if keyword_set(DEBUG) then begin
             print, 'data before trim ' & help, nthedata
             print, 'times before trim ' & help, times
;             print, 'keeping ', q
;             stop;
          endif
         case qq[nqq-1] of 
	    ...
	    nthedata=nthedata[*,q,*]
           ...
           2: begin
	    nthedata=nthedata[*,*,q] ;should always be this case (time is the last array element)
	    end 
         endcase
      endif else print, 'plot_spectrogram: cannot adjust the data array'

       ntimes=ntimes[q]
       ; In the 2-d case, missing last element. add it:
       if (n_elements(dim) eq 2) then ntimes=[ntimes,times[n_elements(times)-1]]
       if keyword_set(DEBUG) then begin
          print, 'DEBUG plot_spectrogram data after trimming'
          help, ntimes
          help, nthedata
       endif
...
; otherwise stuck in this infinite while loop...
      ...
      ; The original version of this line was incorrect.  While it is necessary to check
      ; for fill values to avoid an infinite loop, the variable ntimes holds the original
      ; list of times, not nt, which contains only time differances.
      ; Ron Yurow (June 19, 2017)
      q=where(nt le 0 and ntimes ne Xfillval)
      if q[0] eq -1 then done=1
   endwhile
...
if keyword_set(DEBUG) then print, 'DEBUG in plot_spectrogram, ep16 and eptt = ',ep16, eptt
if keyword_set(DEBUG) then print, 'DEBUG in plot_spectrogram, tbegin and tend = ',tbegin, tend
...
if(igram eq 1) then begin
   ...
   nbad = 0 ; initialize
   if not(eptt) then bad = where(times le 0, nbad) ; can only check times le 0 for non-tt2000 values
...
if(igram eq 1) then begin
   ...
   if(!D.name eq 'Z') then begin 
      ...
      device,set_resolution=sizeWindow,set_colors=256,set_char=[6,11], $
      z_buffering=0
   endif
endif
...
if (keyword_set(AUTO))AND(non_fillcount gt 0) then begin
   ...
   if (num_panels gt 1 and igram ne 1) then begin ; RTB added igram
      ...
      if (a[0] ne -1) then begin
       ...
       ; could have units array where first element is '' but 2nd element is valid string, eg: rbspa_efw-l2_vsvy-hires
       q=where(Zvar.(a[0]) ne '')
       if q[0] ne -1 then begin
          if (Zvar.(a[0])[elist[i]] ne '') then zunits = Zvar.(a[0])[elist[i]]
       endif
        ;might need some day
            ...
            if (sc gt 0) then begin
   	      ...
   	      if (zc gt 0) then begin
   	       ...
   	       mydata[scase] = newmin ;reassign all values that are "valid" but because they
                                ;fall below 0 (w/ log scaling), they
                                ;would be lost.
            endif else begin
               mydata[*,*] = Zfillval ;CWG 04/06/2017 - added to correct for an issue with data values of all 0's on a logarithmic plot. Code now properly informs user there are no good values to display.
            endelse
           endif 
        endif
      ;TJK end of change.
...
if (zmax eq zmin) then begin & zmax = zmax + 1. & zmin = zmin - 1. & endif
   ...
   if(igram) then begin
      ...
      if (desc_str eq 'RPI') then begin
         ...
         if keyword_set(debug) then print, 'TJK Calling spectrogram w/ X and Y min/maxes specified'
