;Author: johnson $
;Date: 2014/08/21 17:59:54 $
;Header: /home/cdaweb/dev/control/RCS/plot_stack.pro,v 1.54 2014/08/21 17:59:54 johnson Exp ryurow $
;Locker: ryurow $
;Revision: 1.54 $
;+------------------------------------------------------------------------
; NAME: PLOT_STACK
...
;Determine the Color bar label
  ...
  if (a[0] ne -1) then begin
    ...
    q=where(Zvar.(a[0]) ne '')
    if q[0] ne -1 then zunits = Zvar.(a[0])
  endif
...
;  if (i eq 0) then begin 
  ...
  if (colr_dims eq 2) then begin
  ;  The following code section had to be rewritten as it will sometimes cause a 
  ;  error if the color data has column that consists only of the fill value.  This
  ;  occurs due the loop ending without finding a valid record.  In this case,
  ;  the record index, clr, will be set to a value outside of the color data array.
  ;  Ron Yurow (Feb 24, 2015)
  ...
  ;  Zt = thecolor[elist,clr]
      ...
      ; Set n_cols to the number color values we will need to find (one for each
      ; plot)
      n_cols = N_ELEMENTS (elist)

      ; Create a default color value.  The defualt color value will be the lowest
      ; non-fill value in the color 
      def_color = MIN (thecolor [WHERE (thecolor ne zfillval)])

      ; Set up the colors array.  For now we will initialize everything to the default
      ; color value.  These values will be replaced by real values.
      colors = REPLICATE (def_color, n_cols)

      ; Loop to find a color for each plot
      FOR col_ind = 0, n_cols - 1 DO BEGIN

          ; Set col to the column in the color data array that we are examining for 
          ; a color
          col = elist [col_ind]

          ; Ge the indices of all the non-fill values in the column.  Hopefully there
          ; will be at least one of these.
          nf = WHERE (thecolor [col, *] ne zfillval, n_non_fill)

          ; If we found a non-fill value, then insert it into the colors array.
          IF n_non_fill gt 0 THEN colors [col_ind] = thecolor [col, nf [0]]

      ENDFOR

      ; Just a copy of colors ...
      Zt = colors
