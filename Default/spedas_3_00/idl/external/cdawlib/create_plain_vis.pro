;Author: johnson $
;Date: 2013/09/06 16:00:15 $
;Header: /home/cdaweb/dev/control/RCS/create_plain_vis.pro,v 1.17 2013/09/06 16:00:15 johnson Exp ryurow $
;Locker: ryurow $
;Revision: 1.17 $
...
; Get the appropiate fill value for image data and initialize the 
; new_image array to that value. This is necessary to handle the case where 
; there is no data.
; Ron Yurow (Sep 2, 2016)
image_fill = astruct.(real_image).FILLVAL
new_image [*] = image_fill

; Get the Epoch variable and Epoch fill_value
; Ron Yurow (Sep 2, 2016)
epoch_var  = tagindex ('EPOCH',atags)
epoch_fill = astruct.(epoch_var).FILLVAL

; Get all the epochs for all the records that we processing.
; Ron Yurow (Sep 2, 2016)
vartags = tag_names(astruct.(epoch_var))
d = tagindex('DAT',vartags)
if (d[0] ne -1) then begin
    epochs = astruct.(epoch_var).DAT 
endif else begin
    d = tagindex('HANDLE', vartags)
    handle_value, astruct.(epoch_var).HANDLE, epochs
endelse

for k = 0, num_recs-1 do begin 

  ; Check to make sure that the time associated with the image is valid
  ; and not set to fill_val.  If it is fill_val then start a new iteration. 
  ; Ron Yurow (Sep 2, 2016)
  IF epochs [k] eq  epoch_fill THEN CONTINUE
