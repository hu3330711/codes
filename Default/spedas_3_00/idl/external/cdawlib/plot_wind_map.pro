...
; RCJ 12/18/2015  Introduced this 'ratio' to make better PS outputs. Note that it is
;           related to the values of xs and ys defined below.
if keyword_set(ps) then ratio=(350./512)*100 else ratio=1.
...
if keyword_set(XSIZE) then xs=XSIZE else begin
   ...
   if (map_proj eq 8) then  xs=512*ratio  ; *ratio  here will work if plot is ps or gif
...
if keyword_set(YSIZE) then ys=YSIZE else begin
   ...
   if (map_proj eq 8) then ys=350*ratio  ; *ratio  here will work if plot is ps or gif
...
;
;if keyword_set(frame) then print,'FRAME is set!!!!!!!!!'  else print,'FRAME is NOT set!!!!!'
;if keyword_set(gif) then print,'gif is set!!!!!!!!!'  else print,'gif is NOT set!!!!!'
;if keyword_set(ps) then print,'ps is set!!!!!!!!!'  else print,'ps is NOT set!!!!!'
idat_orig=idat
glat_orig=glat
glon_orig=glon
...
if keyword_set(FRAME) then begin ; produce plot of a single frame
   ...
   j=frame-1
	 ...
	 ; Option 6 not being used yet. 
      ...
      mylength=10000*ratio ; *ratio will work if plot is ps or gif.
      ; cdaweb_velovect will overplot the wind plot on the map
      cdaweb_velovect,zone,meri,glon,glat,$
        ;latpass=latpass,lonpass=lonpass,$
;TJK adjust to get red        color=230, projection=idl_projection, error=error,$
        color=250, projection=idl_projection, error=error,$
	...
	length=mylength, /device, xy_step=xy_step, $ 
...
;adjust to get red            color=230, projection=idl_projection, position=position, $
            color=240, projection=idl_projection, position=position, $
