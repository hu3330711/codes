...
;Header: /home/cdaweb/dev/control/RCS/plot_images.pro,v 1.78 2015/07/23 17:38:00 johnson Exp ryurow $
;Locker: ryurow $
;Revision: 1.78 $
;+------------------------------------------------------------------------
; NAME: PLOT_IMAGES
...
; The display of VIS images is dependendent on which camera generated the image.
; Therefore, for the VIS instrument we must also determine which camera generated
; the data.
; Ron Yurow (March, 2017)
IF  descriptor[0] eq 'VIS' THEN BEGIN 
    VIS_logical_source = astruct.(vnum).logical_source

    IF  STREGEX (VIS_logical_source, 'EARTH', /FOLD_CASE, /BOOLEAN) THEN BEGIN
        VIS_earth_camera = 1
    ENDIF ELSE BEGIN
        VIS_earth_camera = 0
    ENDELSE
...
; are displayed on the vis home page.  RTB
    ...
    ;The following statement needed to be rewritten so that code can differentiate
    ; between and EARTH CAMERA VIS data and LOW RESOLUTION CAMERA VIS data.  Images
    ; of each instrument are oriented differently.
    ; Ron Yurow (March, 2017)
    ;if(descriptor[0] eq 'VIS') then idat=rotate(rotate(idat,5),2) ; RTB 9/98
    IF  (descriptor[0] eq 'VIS') THEN BEGIN
        IF  VIS_earth_camera THEN BEGIN 
...
;  idat=bytscl(idat,max=idmax,min=idmin,top=!d.table_size-3)+1B
;  w=where(idat gt idmax,wc)
;  if(wc gt 0) then idat[w]=!d.table_size-1 
...
;        ; scale to maximize color spread
;        temp = bytscl(temp,max=max(temp),min=min(temp),top=!d.table_size-1)
;        tv,temp

    endelse
    ; subtitle the plot
  ; project_subtitle,astruct.(0),'',/IMAGE,TIMETAG=edat(FRAME-1)
    ;  RCJ 07/21/2015 Added tcolor to call to project_subtitle.  W/o it the image title was red.
    project_subtitle,astruct.(0),window_title,/IMAGE,TIMETAG=edat[FRAME-1],tcolor=!d.table_size-1
...
;;   idat = bytscl(idat,max=max(idat),min=min(idat),top=!d.table_size-1)
...
;TJK moved this from above the GIF section - so that !d.table_size is correct
...
;  cscale = [zvmin, zvmax]
  ...
  colorbar, cscale, ctitle, logZ=0, cCharSize=cCharSize, $ 
        position=[!x.window[1]+offset,      !y.window[0],$
                  !x.window[1]+offset+0.03, !y.window[1]],$
        fcolor=!d.table_size-1, /image
	; RCJ 07/21/2015   Changed fcolor to match similar change in DeviceOpen
        ;fcolor=244, /image
