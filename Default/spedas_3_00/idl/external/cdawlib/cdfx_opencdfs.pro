...
if mpaths[0] ne '' then begin
  ...
  mast_mess = 'Metadata from '+mpaths[0]+' being used. Modify Preferences if this is not correct.'
  resp = dialog_message(/info, mast_mess)
...
;check for the files w/o data variables and get out
if (n_elements(new_vnames) eq 1 and (new_vnames[0]eq'')) then begin 
  resp = dialog_message(/info, 'No variables w/ var_type = data found.  Select another data file.')
  return, -1
endif
...
if n_elements(vlist) lt 1 then begin ;TJK changed from 2 to 1 on 10/17/2014
  resp = dialog_message(/error, $
