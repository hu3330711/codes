   ...
   3: begin ; Z, X, Y
      ...
      if keyword_set(centerX) then begin
         ...
         for i = 0L, Zsize[2]-1 do begin
            ; Added parameter LOG to align_center.  Ron Yurow (Nov 20, 2015)
            align_center, Xmin[*,i], Xmint, Xmaxt, LOG=LogX
      ...
      if keyword_set(centerY) then begin
         ...
         for i = 0L, Zsize[1]-1 do begin
            ; Added parameter LOG to align_center.  Ron Yurow (Nov 20, 2015)
            align_center, Ymin[i,*], Ymint, Ymaxt, LOG=LogY
...
if (wBadc gt 0) then begin
   ...
   if (wGoodc le 0) then begin ; quit here
      ...
      if doStatus then begin
         ...
         ;Throwing an ERROR message here creates a lot CDAWeb error email about an
         ;Otherwise benign problem -- no data.  Lets not do that.
         ;Ron Yurow (March 22, 2017)
         ;print,'ERROR= Instrument may be off '
         print, 'STATUS= No good values to display for ',parts[0]
         ;status=-1L
         ;return

;TJK 8/10/2015 do this for all panels, otherwise only works for 1st panels
...
; Adjust the xrange and yrange keywords (if they are are included in the extra parameters
; passed to the procedure when it is called by plot_parameters) if either the centerx or 
; the centery flag is set.
;  
; Since each of the center flags causes a shift in how the data is ploted, the values for the 
; xrange and yrange keywords must likewise be adjusted. This is very important in respect to the
; y axis wehre the failure to adjust the y axis range combined with a small number of energy 
; levels will result in the top and bottom bands of the spectrogram being half the width of the 
; other bands.

; Get the all the tags in the extra structure.
extra_names = tag_names(extra)

; Adjust the xrange if the centerx keyword is set.
if  keyword_set (centerx) then begin
 
    ; Check if the XRANGE keyword is set.  If it is then we have to reset it.
    w = where( strupcase (extra_names) eq 'XRANGE', wc)

    if  wc gt 0 then begin 

        ; Find the maximmum x minimum that is smaller than the minimum value that
        ; that was passed as part of the xrange.
        if  max (xmin lt extra.(w[0]) [0]) then begin 
            new_xmin = max (xmin [where (xmin lt extra.(w[0]) [0])])
        endif else begin
            new_xmin = extra.(w[0]) [0]
        endelse

        ; Find the minimum x maximum that is larger than the maximum value that
        ; that was passed as part of the xrange.
        if   max (xmax gt extra.(w[0]) [1]) then begin
             new_xmax = min (xmax [where (xmax gt extra.(w[0]) [1])])
        endif else begin
             new_xmax = extra.(w[0]) [1]
        endelse

        ; Set up the new xrange
        extra.(w[0]) = [new_xmin, new_xmax]

    endif

endif

; Adjust the yrange if the centery keyword is set.
if  keyword_set (centery) then begin
 
    ; Check if the XRANGE keyword is set.  If it is then we have to reset it.
    w = where( strupcase (extra_names) eq 'YRANGE', wc)

    if  wc gt 0 then begin 
       
        ; Find the maximmum y minimum that is smaller than the minimum value that
        ; that was passed as part of the yrange.
        if  max (ymin lt extra.(w[0]) [0]) then begin 
            new_ymin = max (ymin [where (ymin lt extra.(w[0]) [0])])
        endif else begin
            new_ymin = extra.(w[0]) [0]
        endelse

        ; Find the minimum y maximum that is larger than the maximum value that
        ; that was passed as part of the yrange.
        if   max (ymax gt extra.(w[0]) [1]) then begin
             new_ymax = min (ymax [where (ymax gt extra.(w[0]) [1])])
        endif else begin
             new_ymax = extra.(w[0]) [1]
        endelse

        ; Set up the new yrange
        extra.(w[0]) = [new_ymin, new_ymax]
...
if logY and (n_elements(extra) gt 0) then begin
   ; This is moved up in the code, so we don't need to do it here REY.
   ; extra_names = tag_names(extra)
