;Author: kovalick $ 
;Date: 2015/08/17 16:18:31 $
;Header: /home/cdaweb/dev/control/RCS/plot_skymap.pro,v 1.24 2015/08/17 16:18:31 kovalick Exp ryurow $
;Locker: ryurow $
;Revision: 1.24 $
;+------------------------------------------------------------------------
; NAME: PLOT_SKYMAP
...
;    idat = bytscl(idat,min=idmin, max=idmax, top=!d.table_size-2)
    idat = bytscl(idat,min=idmin, max=idmax, top=!d.table_size-1);TJK try to use all colors
...
if (n_images eq 1 )then begin
  ...
  if (strlen(image_title) gt 10) then xyouts,!d.x_size/2,((!d.y_ch_size*2)+10),image_title,/DEVICE,ALIGNMENT=0.5,CHARSIZE=1.4, $
      COLOR=!d.table_size-1  
      ; RCJ 07/21/2015  Changed value of color.  This change had already been made in other programs.
      ;COLOR=244
endif else begin
  if (strlen(image_title[frame-1]) gt 10) then  xyouts,!d.x_size/2,((!d.y_ch_size*2)+10),image_title[frame-1],/DEVICE,ALIGNMENT=0.5,CHARSIZE=1.4, $
      COLOR=!d.table_size-1  
      ; RCJ 07/21/2015  Changed value of color.  This change had already been made in other programs.
      ;COLOR=244
...
xyouts,!d.x_size/2,((!d.y_ch_size*2)+22),tsky,/DEVICE,ALIGNMENT=0.5,CHARSIZE=1.4, $
      COLOR=!d.table_size-1  
      ; RCJ 07/21/2015  Changed value of color.  This change had already been made in other programs.
      ;COLOR=244
...
; idat = bytscl(idat,min=idmin, max=idmax, top=!d.table_size-2)
 idat = bytscl(idat,min=idmin, max=idmax, top=!d.table_size-1) ; TJK use all colors
...
;                 !x.window[1]+offset+0.03, !y.window[1]],$
        fcolor=!d.table_size-1, /image
	; RCJ 07/21/2015  Changed fcolor to match change in DeviceOpen
        ;fcolor=244, /image
