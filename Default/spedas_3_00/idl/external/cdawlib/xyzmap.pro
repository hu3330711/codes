;Author: johnson $
;Date: 2016/12/08 18:56:53 $
;Header: /home/cdaweb/dev/control/RCS/xyzmap.pro,v 1.54 2016/12/08 18:56:53 johnson Exp johnson $
;Locker: johnson $
;Revision: 1.54 $
;+
; NAME: XYZMAP.PRO  
...
; syrs1=strtrim(string(yrs1),2)
  ...
  idate=idat0+blk+shr0+":"+smn0+bl2+idat1+blk+shr1+":"+smn1
...
;   msymsiz=symsiz*25
    ...
    if(pltsym eq 0) then sym=1
    if(pltsym eq 1) then sym=2
    if(pltsym eq 2) then sym=3
    if(pltsym eq 3) then sym=4
    if(pltsym eq 4) then sym=5
    if(pltsym eq 5) then sym=6
    if(pltsym eq 6) then sym=7
    if(pltsym eq 7) then sym=1
    if(pltsym eq 8) then sym=2
    if(pltsym eq 9) then sym=3
    if(pltsym eq 10) then sym=4
    if(pltsym eq 11) then sym=5
    if(pltsym eq 12) then sym=6
...
if(sym eq 1) then thissym='+'
if(sym eq 2) then thissym='*'
if(sym eq 3) then thissym='.'
if(sym eq 4) then thissym='!9V!x'
if(sym eq 5) then thissym='!7D!x'
if(sym eq 6) then thissym='!9B!x'
if(sym eq 7) then thissym='x'
xyouts, xoffset,yoffset,thissym,/norm,color=symcol,charsize=symsiz
