...
FORMAT10 ='(//1X,"****RECALC WARNS:  YEAR IS OUT OF INTERVAL 1965-2020: IYR=",I4,/,6X,"CALCULATIONS WILL BE DONE FOR IYR=",I4,/)'
...
;C
      ...
      IF(IY GT 2020)THEN IY=2020
;C
;C  (old comment) WE ARE RESTRICTED BY THE INTERVAL 1965-2015,
;C  WE ARE RESTRICTED BY THE INTERVAL 1965-2020,
...
;TJK added use of FLOAT	   F2=(IY+IDAY/365D0-1995.)/5.
	...
	ENDIF ELSE IF (IY LT 2015) THEN BEGIN		;!2010-2015
           F2=(FLOAT(IY)+FLOAT(IDAY-1)/365.25-2010.)/5.
	   F1=1.-F2
	   G10=29496.57*F1+29442.0*F2
	   G11=-1586.42*F1-1501.0*F2
	   H11=4944.26*F1+4797.1*F2
;
;   LINEAR EXTRAPOLATION BEYOND 2015 BY USING SECULAR VELOCITY COEFFICIENTS:
;
	ENDIF ELSE BEGIN				;!2015-2020
;TJK added use of float and divide by 366 instead of 365
;	   DT=IY+IDAY/365D0-2010.
           DT=FLOAT(IY)+FLOAT(IDAY-1)/365.25-2015.
	   G10=29442.0-10.3*DT      ;! HERE G10 HAS OPPOSITE SIGN TO THAT IN IGRF TABLES
	   G11=-1501.0+18.1*DT  ; multiplying by DT are elements from igrf??s.dat: -G10 (yes,negative), 
	   H11=4797.1-26.6*DT    ; G11 and H11.
	ENDELSE
;C
