...
yRightLabel = plotmerge eq 1? ylabel: $
  ...
  STRUPCASE((STRSPLIT( variable.source_name,'>',/EXTRACT))[0])
...
;+------------------------------------------------------------------------
;$Author: rchimiak $
;$Date: 2017/04/24 20:02:34 $
;$Header: /home/cdaweb/dev/control/RCS/plot_over.pro,v 1.41 2017/04/24 20:02:34 rchimiak Exp rchimiak $
;$Locker: rchimiak $
;$Revision: 1.41 $
;+------------------------------------------------------------------------
; Description:
...
if (plotmerge eq 2) then begin
  ...
  for j=0,num_panels-1 do begin
    ...
    single = 0
    source = STRUPCASE((STRSPLIT( Yvar.(0).source_name,'>',/EXTRACT))[0])
    for i=1,iter_per_panel do begin
      single =  STRCMP(source, STRUPCASE((STRSPLIT( Yvar.(i).source_name,'>',/EXTRACT))[0]))
    endfor
  ...
  if (nogood_counter[j] eq 0) then begin
    ...
    if keyword_set(onlylabel) then begin
     ...
     s =  overlay (i,iter_per_panel,*thetimes_ptr[i],*mydata[i],YUNIT = labels.yLeftLabel,YTITLE= labels.yRightLabel,YRANGE=yranger,$
        ...
        NOERASE=clear_plot,_EXTRA=EXTRAS,ytick_get=yticks,psize,color=mycolor,psym, first)
    endelse
