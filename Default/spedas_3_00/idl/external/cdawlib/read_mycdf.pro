...
;       mathews@nssdc.gsfc.nasa.gov  read_myCDF.pro  (301)286-6879
; MODIFICATION HISTORY:
;-------------------------------------------------------------------------
...
; Search through all the z variables in the CDF to see if any of their
; names match the name of the variable we going to try to read.  As
; soon as we find a match, break out of the loop.
; Ron Yurow (4 Nov 2016)
FOR vr = 0, cinfo.nzvars - 1 DO BEGIN
    vinfo = cdf_varinq (CDFid, vr, /ZVARIABLE) 

    IF  STRCMP (vinfo.name, vname) THEN BEGIN
        vfound = 1
        BREAK  
    ENDIF     
ENDFOR

; If we didn't find a match, then search through all the r variables 
; in the CDF to see if any of their names match the name of the variable
; we going to try to read.  Again, as soon as we find a match, break out
; of the loop.
; Ron Yurow (4 Nov 2016)
IF  ~ vfound THEN BEGIN
    FOR vr = 0, cinfo.nvars - 1 DO BEGIN        
        vinfo = cdf_varinq (CDFid, vr, /ZVARIABLE) 
...
; We don't need this line anymore, as we already read the requested info.
; Ron Yurow (4 Nov 2016)
