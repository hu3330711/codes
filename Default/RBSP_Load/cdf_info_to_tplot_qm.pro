...
;-
pro cdf_info_to_tplot_qm,cdfi,varnames,loadnames=loadnames,  $
...
;          var_1 = var_2        ;bpif  v.name eq 'thb_sir_001'
     ...
     units = struct_value(attr,'units',default='')
      if keyword_set(var_2) then begin
        if ~keyword_set(var_1) then data = {x:tvar.dataptr,y:v.dataptr,v1:var_2.dataptr,v2:var_2.dataptr} $
          else data = {x:tvar.dataptr,y:v.dataptr,v1:var_1.dataptr, v2:var_2.dataptr}
      endif else if keyword_set(var_1) then data = {x:tvar.dataptr,y:v.dataptr, v:var_1.dataptr}  $
