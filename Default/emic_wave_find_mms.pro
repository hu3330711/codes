pro emic_wave_find_mms,sc,year,month
  ...
  ;-------
    ...
    tinterpol_mxn,'mms'+probe+'_mec_l_dipole','mms'+probe+'_fgm_b_fac_srvy_l2_perp_dpwrspc',/overwrite
    get_data,'mms'+probe+'_fgm_bt_srvy_l2',data=bt
    get_data,'mms'+probe+'_mec_l_dipole',data=l
    
    get_data,'mms'+probe+'_fgm_b_fac_srvy_l2_perp_dpwrspc',data=tmp,dlim=dlim
    for tmpind=0,n_elements(tmp.x)-1 do begin
      if l.y[tmpind] gt 10.0 or l.y[tmpind] lt 2.0 then begin
        tmp.y[tmpind,*]=!values.F_nan
      endif
    endfor
    store_data,'mms'+probe+'_fgm_b_fac_srvy_l2_perp_dpwrspc',data=tmp,dlim=dlim
...
; write_csv
    ...
    options,'mms'+probe+'_mec_mlat','ytitle','MLAT'
    ...
    popen,'/projectnb/burbsp/home/xcshen/MMS_'+sc+'_EMIC_identify_'+strmid(tout,0,10)+'T'+strmid(tout,11,2)+strmid(tout,14,2),xsize=8,ysize=9,units=cm,/encapsulated
      tplot,['mms'+probe+'_fgm_bt_srvy_l2','SpectralDensity','SpectralDensity_median','Ratio','dot'],trange=[st,et],var_label=['mms'+probe+'_mec_mlat','mms'+probe+'_mec_mlt','mms'+probe+'_mec_l_dipole'];['2007-07-04/13:30','2007-07-04/16:00']
