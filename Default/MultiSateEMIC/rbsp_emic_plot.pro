PRO rbsp_emic_plot,sc,st,et
  ...
  fig_dir='/projectnb/burbsp/home/xcshen/Identified_EMIC_wave_forth_result/'
  file_mkdir,fig_dir
  file_mkdir,fig_dir+'png/'
  file_mkdir,fig_dir+'eps/'
  file_mkdir,fig_dir+'removed/'+'png/'
  file_mkdir,fig_dir+'removed/'+'eps/'

  
  if time_double(et)-time_double(st) lt 2.*60 then return
  ...
  if mediandeg gt 0.7 then begin
    ...
    popen,fig_dir+'eps/'+'RBSP_'+sc+'_EMIC_wave_polorization_'+strmid(st,0,10)+'T'+strmid(st,11,2)+strmid(st,14,2),xsize=8,ysize=9,units=cm,/encapsulated
    ...
    spawn,'convert -density 90 -background "#FFFFFF" -flatten '+fig_dir+'eps/'+'RBSP_'+sc+'_EMIC_wave_polorization_'+strmid(st,0,10)+'T'+strmid(st,11,2)+strmid(st,14,2)+'.eps '+fig_dir+'png/'+'RBSP_'+sc+'_EMIC_wave_polorization_'+strmid(st,0,10)+'T'+strmid(st,11,2)+strmid(st,14,2)+'.png'

    start_time=[start_time,st]
    end_time=[End_time,et]
  endif else begin
    popen,fig_dir+'removed/'+'eps/'+'RBSP_'+sc+'_EMIC_wave_polorization_'+strmid(st,0,10)+'T'+strmid(st,11,2)+strmid(st,14,2),xsize=8,ysize=9,units=cm,/encapsulated
    tplot,['rbsp'+sc+'_SpectralDensity','rbsp'+sc+'_WNA','rbsp'+sc+'_Ellipticity'],trange=trange_plot,$
      var_label=['rbsp'+sc+'_ect_mageis_L3_MLAT','rbsp'+sc+'_ect_mageis_L3_MLT','rbsp'+sc+'_ect_mageis_L3_L']
    pclose

    spawn,'convert -density 90 -background "#FFFFFF" -flatten '+fig_dir+'removed/'+'eps/'+'RBSP_'+sc+'_EMIC_wave_polorization_'+strmid(st,0,10)+'T'+strmid(st,11,2)+strmid(st,14,2)+'.eps '+fig_dir+'removed/'+'png/'+'RBSP_'+sc+'_EMIC_wave_polorization_'+strmid(st,0,10)+'T'+strmid(st,11,2)+strmid(st,14,2)+'.png'
